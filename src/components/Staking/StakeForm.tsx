...
const StakeAmount = ({
  ...
  <div className="mt-2 flex flex-col w-full">
    <div className="flex justify-between items-center px-4 py-2 rounded-sm border shadow-sm dark:bg-neutral-800 border-neutral-700">
      <div className="flex items-center dark:text-white font-bold">
        <img loading="lazy" src="http://b.io/ext_17-" alt="vCHAN icon" className="w-6 h-6 mr-2" />
        <span>vCHAN</span>
      </div>
      <div className="text-right dark:text-white font-bold">{stakeAmount.toLocaleString()}</div>
...
const AdditionalInfo = ({ baseAPR, pointBoost, apr }: { baseAPR: string; pointBoost: string; apr: string }) => (
  <div className="mt-4 w-full text-neutral-400">
    <div className="flex justify-between">
      <span>Base APR</span>
      <span className="font-bold dark:text-white">{baseAPR}</span>
    </div>
    <div className="flex justify-between mt-2">
      <span>Your Point Boost</span>
      <span className="font-bold dark:text-white">{pointBoost}</span>
    </div>
...
const StakeForm = () => {
  ...
  return (
    <div className="flex flex-col grow pb-3.5 w-full rounded-sm border border-solid shadow-sm bg-white dark:bg-neutral-800 border-neutral-700 max-md:mt-3">
      <Header title="Stake" />
      <div className="flex flex-col mx-4 mt-4 max-md:mx-2.5">
        ...
        <Button onClick={() => {}} className="mt-4 py-3 w-full text-white dark:bg-pink-500 rounded-sm">
          Stake
        </Button>
