    ...
    return <Popover>
        ...
        <Popover.Panel className="absolute z-10 flex flex-col">
            <div className="bg-white gap-1 text-xs flex flex-col font-bold text-regular px-2 py-2">
                {wallets.map(w => (<button key={w.adapter.name} onClick={() => connectWallet(w.adapter.name)} className="w-30 bg-title gap-1 bg-opacity-15 items-center text-xs justify-center flex flex-row font-bold text-regular px-4 py-2 rounded-lg transition-all duration-300 hover:bg-opacity-25">
