...
import { BE_URL_DEV, TokenApiHelper } from "@avernikoz/memechan-sol-sdk";
...
const fetchPresaleCoinUniqueHoldersFromBE = async (memeMint: string) => {
  try {
    const holdersMap = await TokenApiHelper.getBondingPoolHoldersMap(new PublicKey(memeMint), BE_URL_DEV);
