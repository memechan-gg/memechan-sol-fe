...
export function CreateCoin() {
  ...
  const { mainTokenThresholdAmount } = useTargetConfig();
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      if (inputAmountIsSpecified) {
        if (!mainTokenBalance) return toast.error(`You need to have ${mainTokenName} for initial buy`);

        const amountBigNumber = new BigNumber(inputAmount);
        const thresholdWithFees = mainTokenThresholdAmount
          ? new BigNumber(mainTokenThresholdAmount).multipliedBy(1.01)
          : null;
        ...
        if (amountBigNumber.gt(mainTokenBalance)) return toast.error("Insufficient balance");

        if (thresholdWithFees && amountBigNumber.gt(thresholdWithFees))
          return toast.error(
            `The maximum ${mainTokenName} amount to invest in bonding pool is ${thresholdWithFees.toPrecision()} ${mainTokenName}`,
          );
      }
        ...
        <ThreadBoard title="create memecoin">
          <form onSubmit={onSubmit} className="flex flex-col gap-4 lowercase">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex flex-col lg:flex-row gap-4 flex-wrap">
                <div className="flex flex-col gap-1">
                  <label className="text-regular text-xs">{mainTokenName} amount</label>
                  <div>
                    <input
                      className="border w-[200px] border-regular rounded-lg p-1"
                  ...
                  <span className="text-regular">
                    {mainTokenName} available: {publicKey ? mainTokenBalance ?? <Skeleton width={40} /> : 0}
                  </span>
                </div>
              </div>
            </div>
